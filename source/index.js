...
export function initApp() {
  ...
  const reviewGreeting = document.getElementById("reviewGreeting");
  const ratingStars = document.getElementById("ratingStars");
  const ratingHint = document.getElementById("ratingHint");
  const reviewText = document.getElementById("reviewText");
  const reviewSubmit = document.getElementById("reviewSubmit");
  const reviewSubmitHint = document.getElementById("reviewSubmitHint");
  const reviewsList = document.getElementById("reviewsList");
  const reviewsEmpty = document.getElementById("reviewsEmpty");
  const reviewsRefresh = document.getElementById("reviewsRefresh");

  let selectedRating = 0;

  function starsHtml(n) {
    const full = "★".repeat(n);
    const empty = "☆".repeat(6 - n);
    return `<span class=\"font-semibold\">${full}${empty}</span>`;
  }
  ...
  function renderStarPicker() {
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      b.type = "button";
      b.className = "h-10 w-10 rounded-2xl border border-black/10 bg-white/70 dark:bg-white/5 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10 flex items-center justify-center text-xl";
      b.dataset.star = String(i);
      b.textContent = i <= selectedRating ? "★" : "☆";
      b.addEventListener("click", () => {
        selectedRating = i;
        renderStarPicker();
        if (ratingHint) ratingHint.textContent = `Rating kamu: ${selectedRating}/6`;
      });
  ...
  async function loadReviews() {
    ...
    try {
      ...
      for (const it of items) {
        ...
        card.className = "rounded-2xl border border-black/10 bg-black/5 dark:border-white/10 dark:bg-white/5 p-4";

        const when = it.created_at ? new Date(it.created_at).toLocaleString("id-ID") : "";
        const verified = it.verified ? `<span class=\"ml-2 rounded-full px-2 py-0.5 text-[11px] border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-200\">Verified</span>` : "";

        card.innerHTML = `
          <div class=\"flex items-start gap-3\">
            <img src=\"./img/profil.svg\" alt=\"profil\" class=\"h-10 w-10 rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5\" />
            <div class=\"flex-1\">
              <div class=\"flex items-center justify-between gap-3\">
                <div class=\"font-semibold\">${escapeHtml(it.username || "User")}${verified}</div>
                <div class=\"text-xs text-zinc-600 dark:text-zinc-300/80\">${escapeHtml(when)}</div>
              </div>
              <div class=\"mt-1 text-sm text-zinc-800 dark:text-zinc-200/90\">${starsHtml(Number(it.rating || 0))}</div>
              <div class=\"mt-2 text-sm text-zinc-700 dark:text-zinc-300/90 leading-relaxed\">${escapeHtml(it.comment || "")}</div>
            </div>
          </div>
        `;
  ...
  function showReviewForm() {
    if (!reviewFormWrap) return;
    const uname = getSavedUsername() || "User";
    reviewGreeting.textContent = `Halo ${uname}`;
    reviewFormWrap.classList.remove("hidden");
    reviewSubmitHint.textContent = "";
    selectedRating = 0;
    reviewText.value = "";
    renderStarPicker();
    if (ratingHint) ratingHint.textContent = "Pilih bintang dulu.";
  }

  async function submitReview() {
    if (!state.order) {
      toast("Order belum ada.", "error");
      return;
    }
    const uname = getSavedUsername();
    const comment = (reviewText?.value || "").trim();
    if (!uname) return toast("Username belum ada.", "error");
    if (!selectedRating) return toast("Pilih rating dulu (1-6).", "error");
    if (!comment) return toast("Ulasan belum diisi.", "error");

    reviewSubmit.disabled = true;
    reviewSubmitHint.textContent = "Mengirim ulasan...";
    try {
      ...
      reviewSubmitHint.textContent = "Ulasan terkirim ✅";
      reviewFormWrap.classList.add("hidden");
      await loadReviews();
    } catch (e) {
      toast(e?.message || "Gagal kirim ulasan", "error");
      reviewSubmitHint.textContent = e?.message || "Gagal";
    } finally {
      reviewSubmit.disabled = false;
    }
  }

  if (reviewsRefresh) reviewsRefresh.addEventListener("click", () => loadReviews());
  ...
  async function refreshStatus() {
    ...
    try {
      ...
      if (looksPaid(status)) {
        ...
        // Show rating & review form after successful payment (one time per order)
        try {
          const rk = `reviewPrompted:${state.order.orderId}`;
